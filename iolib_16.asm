;[bits 16]
print_string:
	mov ah, 0x0e ; ensure we are in tele-type mode
	pusha ; save all current registers
	print_loop: ; main print loop
		mov cx, [bx] ; store the value at the address in bx to cx
		cmp cl, 0 ; check if we hit null termination
		je else
		mov al, cl ; move cl to al for printing
		int 0x10 ; call the video interrupt
		add bx, 0x1 ; increment our address by one so next value
					; in string can be printed
		jmp print_loop ; keep printing
		else:
			popa ; restore registers to what they were
			ret ; return to ip
